...
if [ -n "$3" ]; then
    echo -e "${CYAN}Using custom PR description${NC} ${YELLOW}(fancy!)${NC}"
fi
if [ -z "$1" ] && [ -z "$2" ] && [ -z "$3" ]; then
...
if command -v gh &> /dev/null; then
    # Create PR description (use custom or default)
    if [ -n "$PR_DESCRIPTION" ]; then
        # Use custom description but ensure required sections exist
...
*Powered by attitude and caffeine* ☕"
        ...
        # Use default snarky description
        PR_BODY="## 🎯 What's This?
